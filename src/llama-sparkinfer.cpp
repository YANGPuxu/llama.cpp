...
#include "ggml-cuda.h" // For ggml_backend_cuda_context, cudaStream_t and cudaMemcpyAsync
// #include "common.cuh" // For CUDA_CHECK macro
...
extern "C" {  
    ...
    bool ggml_spif_reload_exec(ggml_spif_context* ctx, ggml_tensor * tensor) {  
        sparkInfer_layer_cache * spif_cache = reinterpret_cast<sparkInfer_layer_cache*>(ctx);  
        return spif_cache->spif_reload_exec(tensor, *cuda_ctx);
    }
}
...
bool sparkInfer_layer_cache::spif_reload_exec(ggml_tensor * tensor, ggml_backend_cuda_context & ctx) {

    // 1. Get the Stream from ctx
    // [YPX] [Todo] Check the real id of our I/O stream. Here we use stream id=1 as the I/O stream.
    cudaStream_t io_stream = ctx.stream(ctx.device, 1);

    // 2. Get the "Reload Plan" from 'this' (spif_cache)
    ...
    GGML_ASSERT(groups_to_reload.size() == slots_for_evict.size());

    // 3. From 'tensor' (the operator node), get the "Reload Targets"
    struct ggml_tensor * gpu_cache_tensor = tensor->src[0]; // e.g., gpu_ffn_up_cache
    struct ggml_tensor * cpu_weight_tensor = tensor->src[1]; // e.g., cpu_ffn_up

    // 4. Calculate parameters needed for offsets
    ...
    for (size_t i = 0; i < groups_to_reload.size(); ++i) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(dst_addr, src_addr, group_size_bytes, cudaMemcpyHostToDevice, io_stream));
    }
