...
[YPX] [Q] 为什么是 vector 列表呢，直接用 layer_cache * 不行吗？还是说 cache 里面可以使用 C++ 语法？
...
1. **GPU 上需要动态调整的神经元组**：
    ...
    `ffn_gpu_gate`
    `ffn_gpu_down_t`
2. **指导 GPU 神经元映射、reload 的张量**：
    `gpu_neu_idx/mask`（其中 `gpu_neu_idx` 在 GPU 上）
    `DFR_score`
3. **神经元组到神经元的双向映射**：
    `group_to_neurons_map`（维度为 `[layer_group_count, layer_group_size]`）
    `neuron_to_group_map`
4. **用于快速查阅当前有哪些神经元组在 GPU 上的张量**：
    `group_to_slot_map` 和 `slot_to_group_map`
    思考：`group_to_slot_map` 是不是必需的呢？我感觉是必需的，因为如果 CPU 端只知道哪些组在 GPU 上而不知道具体位置，那就无法确定把新的组搬到哪个位置上，没法做到精准 evict 和 reload
5. **其他**：
    一些 hyperparameter (`n_layers`, `dfr_decay`...)
6. **(optional)** 构建 reload 的计算图
