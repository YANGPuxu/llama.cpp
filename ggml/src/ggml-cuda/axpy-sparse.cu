...
static void launch_mul_mat_axpy_cuda_sparse(const T *       x,
    ...
    if (src_ncols == 1) {
        ...
        mul_mat_axpy_sparse<<<block_nums, block_dims, ncols * sizeof(float), stream>>>(x, y, dst, ncols, nrows,
                                                                                       gpu_neu_idx, sparse_idx);
    } else {  // batch_axpy
        const dim3 block_nums(num_blocks, src_ncols, 1);
        const dim3 block_dims(WARP_SIZE, 1, 1);
        mul_mat_axpy_sparse_batch<<<block_nums, block_dims, ncols * sizeof(float), stream>>>(x, y, dst, ncols, nrows,
                                                                                             gpu_neu_idx, sparse_idx);
    }
}
